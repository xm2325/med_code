...
def write_cadec_report(output_dir: str | Path) -> Path:
    ...
    manifest = _read_json(benchmark / "experiment_manifest.json")
    provenance = _read_json(benchmark / "model_provenance.json")
    explanation_quality = _read_json(benchmark / "explanation_quality.json")
    ...
    model_selection = _read_csv(benchmark / "model_selection.csv")
    memory = _read_json(benchmark / "historical_memory_value.json")

    version = str(manifest.get("version", "0.0.13"))
    metric_rows = [
        ...
        ("Selected dense weight", metrics.get("selected_dense_weight")),
        ("Selected reranker weight", metrics.get("selected_reranker_weight")),
        ("Accuracy@1 delta vs terminology-only", metrics.get("advanced_accuracy_delta", metrics.get("historical_memory_accuracy_delta"))),
    ]
...
<h1>MedCode v{escape(version)} — CADEC → MedDRA</h1>
...
<div class='warning'><b>Interpretation guard:</b> model family, fusion weights and AUTO/HUMAN_REVIEW thresholds are selected on validation only. TEST is final held-out evaluation, not a tuning source.</div>
<h2>Pre-flight dataset readiness</h2><pre>{escape(json.dumps(audit, indent=2)) if audit else 'No dataset audit artifact found.'}</pre>
<h2>Held-out TEST coding metrics</h2>{metric_table.to_html(index=False)}
<h2>Validation-only model selection</h2>{model_selection.to_html(index=False) if not model_selection.empty else '<p>No model-selection output.</p>'}
<h2>Model provenance</h2><pre>{escape(json.dumps(provenance, indent=2)) if provenance else 'Lexical baseline / no separate model provenance artifact.'}</pre>
...
<h2>Historical coding memory / advanced-model value</h2><pre>{escape(json.dumps(memory, indent=2))}</pre>
<h2>Explanation quality gate</h2><pre>{escape(json.dumps(explanation_quality, indent=2)) if explanation_quality else 'Run the explanation stage to generate explanation_quality.json.'}</pre>
<h2>Explainability and expert review</h2>
<p>Inspect <code>explanations.html</code> for code/evidence/why cards, <code>explanation_quality_cases.csv</code> for per-case quality failures/warnings, and <code>../casebook/review_casebook.html</code> for priority expert-review cases with original context.</p>
</body></html>"""
