...
def validate_multi_candidate_payload(
    payload: Mapping[str, Any],
    *,
    allowed_codes: Iterable[str],
    allowed_evidence_quotes: Iterable[str],
) -> tuple[bool, list[str]]:
...
class DeepSeekRealCandidateEvaluator:
    ...
    def evaluate_case(
        ...
        *,
        ...
        candidate_grounding: list[dict[str, Any]],
        ...
        system = (
            "You are evaluating a FIXED set of medical coding candidates for a PUBLIC benchmark phrase. "
            "Preserve exactly the supplied code set; do not add, delete, merge, or duplicate codes. "
            "Rerank the candidates and provide a separate concise rationale for EVERY candidate. "
            "Rationales must distinguish support from ambiguity and may only cite evidence quotes copied exactly from allowed_source_evidence. "
            "Do not invent clinical facts. Return JSON only with ranked_codes, overall_uncertainty, and candidate_rationales."
        )
        user = {
            ...
            "required_schema": {
                ...
                "candidate_rationales": [
                    {"code": code, "rationale": "why this option fits or is less supported", "evidence_quotes": [str(phrase)]}
        ...
        for _ in range(max(1, int(max_attempts))):
            try:
                ...
                continue
            valid, errors = validate_multi_candidate_payload(
                payload,
                allowed_codes=allowed_codes,
                allowed_evidence_quotes=allowed_quotes,
            )
            if valid:
