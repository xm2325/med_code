...
UI_TRANSLATIONS = {
    ...
    "；支持离线选择、随机抽取和中英文合成输入。请勿粘贴真实患者资料。": "; supports offline selection, random sampling, and synthetic input in English or Chinese. Do not paste real patient data.",
    ...
    "同一条记录调用三种真实后端方法并排比较。只有带 LIVE 或 REPLAY 标记的结果才是模型实际返回；OFFLINE 不展示预制答案。": "Compare three real backend methods side by side for the same record. Only LIVE or REPLAY results are actual model returns; OFFLINE never shows canned answers.",
    "尚未调用后端": "Backend not called yet",
    "Qwen 直接结构化抽取并在公开 LLT allowlist 内选择；机器校验 code、quote 与 offset。": "Qwen extracts structured data and selects within the public LLT allowlist; machine checks validate code, quote, and offset.",
    "LLM + 词法检索": "LLM + lexical retrieval",
    "Qwen 提取事件和语境；使用 exact、local alias 与 fuzzy 在 MedDRA 28.0 公开子集中检索。": "Qwen extracts event and context; exact, local alias, and fuzzy matching retrieve from the public MedDRA 28.0 subset.",
    "Qwen 提取事件和语境；使用 exact、local alias 与 fuzzy 在 NCI CTCAE v6.0 公开子集中检索（该来源含 MedDRA 28.0 LLT 字段）。": "Qwen extracts event and context; exact, local-alias, and fuzzy matching retrieve from the public NCI CTCAE v6.0 subset carrying MedDRA 28.0 LLT fields.",
    "标准 BM25 + character trigram + exact boost 检索并冻结 Top-K；Qwen 只能重排，不能新增、删除或修改候选 code。": "Standard BM25 + character trigram + exact boost retrieve and freeze Top-K; Qwen may only rerank and cannot add, remove, or alter candidate codes.",
    "BM25 + character trigram + exact boost 检索并冻结 Top-K；Qwen 只能重排，不能新增、删除或修改候选 code。": "BM25 + character trigram + exact boost retrieval freezes Top-K; Qwen may only rerank and cannot add, delete, or modify candidate codes.",
    "Qwen 提取事件和语境；exact、alias、BM25 与 trigram 在 MedDRA 28.0 公开子集中检索。": "Qwen extracts event and context; exact, alias, BM25, and trigram retrieve from the public MedDRA 28.0 subset.",
    "混合检索冻结 Top-K；Qwen 只能重排，不能新增、删除或修改候选 code。": "Hybrid retrieval freezes Top-K; Qwen may only rerank and cannot add, delete, or modify candidate codes.",
    "本地匹配分数（非概率）": "Local match score (not a probability)",
    "本地匹配分数 0.00（非概率）": "Local match score 0.00 (not a probability)",
    "后端不可用；未展示模型结果": "Backend unavailable; no model results shown",
    "真实后端返回": "Real backend return",
    "后端尚未运行。点击“生成编码建议”调用真实 API。": "Backend has not run. Click Generate coding suggestion to call the real API.",
    "正在连接真实后端…": "Connecting to the real backend...",
    "已选择记录；点击“生成编码建议”调用真实后端。": "Record selected; click Generate coding suggestion to call the real backend.",
    "没有实际返回。不会用设计示例或本地预制结果替代模型输出。": "No actual return. Design examples and canned local results never replace model output.",
    "无返回": "No return",
    "后端响应未包含": "Backend response does not include",
    "该方法未执行；code=null，candidates=[]。": "This method did not run; code=null and candidates=[].",
    "该方法未执行或执行失败；code=null，candidates=[]。": "This method did not run or failed; code=null and candidates=[].",
    "方法返回验证错误；不会显示或采用任何代码。": "The method returned validation errors; no code is displayed or accepted.",
    "验证错误": "Validation error",
    "全部事件抽取": "All extracted events",
    "各事件检索 / 候选": "Per-event retrieval / candidates",
    "事件": "Event",
    "公共来源未提供": "Not provided by the public source",
    "或规则": "or rules",
    "抽取": "Extraction",
    "检索 / 候选": "Retrieval / candidates",
    "验证": "Validation",
    "反馈说明（仅限合成数据）": "Feedback note (synthetic data only)",
    "不要输入患者姓名、病历号或其他真实患者信息。": "Do not enter names, record numbers, or other real patient information.",
    "后端不可用（": "Backend unavailable (",
    "）；未展示模型结果。": "); no model results shown.",
    "真实 API 不可用，模型结果保持 OFFLINE": "Real API unavailable; model results remain OFFLINE",
}
